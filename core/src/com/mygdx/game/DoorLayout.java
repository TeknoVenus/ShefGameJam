...
public class DoorLayout {
	...
	private Texture doorBrokeTexture;
	private Sprite doorBrokeSprite;
	...
	public DoorLayout(RoomRepresentation room, SpriteBatch batch) {
		...
		doorBrokeTexture = new Texture(Gdx.files.internal("textures/doorBroke.png"));
		doorBrokeSprite = new Sprite(doorBrokeTexture);
	}
	...
	public void draw(Rectangle door, boolean side, boolean isBroken) {
		batch.begin();
		if (isBroken) {
			batch.draw(doorBrokeSprite, door.getX(), door.getY(), door.getWidth(), door.getHeight());
		} else {
			if (!side)
